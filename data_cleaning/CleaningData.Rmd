...
importf(streamList, ".txt", "./data_original/", "_stream_chem")
...
precip_stream_data %<>% group_by(ws, water_year, source, solute) %>%
  ...
  mutate(water_mm_py = sum(water_mm_pm))

precip_stream_data %>% 
  filter(ws == "6") %>% 
  select(water_year, ws, water_mm_pm, water_mm_py)
...
```{r}
save(precip_stream_data, file = "precip_stream_data_long.Rdata")
save(precip_stream_data_wide, file = "precip_stream_data_wide.Rdata")
save(precip_stream_diff_data_long, file = "precip_stream_diff_data_long.Rdata")


load("precip_stream_diff_data_long.RData")
  load("precip_stream_data_long.RData")
  load("precip_stream_data_wide.RData")

  save(list = ls(all.names = TRUE), file = "precip_stream_dfs.RData", envir = .GlobalEnv)
```

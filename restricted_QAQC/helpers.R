...
prep_stickytrap_data = function(input, graphnum){
    ...
    maybe_site = ifelse(graphnum %in% 3:4, 'site', 'not gonna match')
...
get_buffered_yrange <- function(d){
    ...
    d <- select(d, -any_of(c('date', 'Hydro.med', 'Flow_or_Precip')))
    dvar <- colnames(d)
    is_ph <- length(dvar) == 1 && dvar == 'pH'
...
validate_formula <- function(formula) {
    # Define a regex pattern that allows only numbers, letters, spaces, and arithmetic operators
    if(grepl("^[0-9A-Za-z\\+\\*\\/ \\-]+$", formula)) {
        return(formula)
    } else {
        stop('Invalid input: only alphnumerics, spaces, and +-*/ allowed.')
    }
}

parse_composite_factor <- function(composite_str) {
    
    composite_str <- validate_formula(composite_str)
    components <- strsplit(composite_str,
                           "(?<=[0-9A-Za-z\\-])\\s+(?=[\\+\\*\\/\\-])|(?<=[\\+\\*\\/\\-])\\s+(?=[0-9A-Za-z\\-])",
                           perl = TRUE)[[1]] %>% 
        str_trim()
    
    missing_space <- grepl('[\\+\\*/\\-].+|.+[\\+\\*/\\-]', components)
    jk <- grepl('N[OH][34]\\-N', components)
    if(any(missing_space & ! jk)){
        # showNotification('Spaces required around arithmetic operators.',
        #                  type = "error")
        stop('Spaces required around arithmetic operators in Composite Factor')
    }
    
    if(any(grepl('^[\\+\\*/\\-]$', components[c(1, length(components))]))){
        stop('Leading/trailing arithmetic operator in Composite Factor')
    }
    
    out <- list(solutes = str_subset(components, '[^\\+\\*\\/\\-]'),
                expression = paste(components, collapse = ' '))

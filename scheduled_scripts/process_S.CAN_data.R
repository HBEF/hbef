...
con <- dbConnect(driver,
  user = "root", password = pass, host = "localhost",
  dbname = dbname
)
...
d1 <- read.csv("batches_1_and_2.csv",
  ...
  rename(
    datetime = 1, Nitrate_mg = 2, Nitrate_mg_status = 3, TurbidityRaw = 4,
    TurbidityRaw_status = 5
  ) %>%
  mutate(datetime = as.POSIXct(datetime,
    tz = "EST"
...
clean_SCAN_data <- function(fpath) {
  read_xlsx(fpath, skip = 4, guess_max = 10000, col_names = FALSE) %>%
    select(-(4:5)) %>% # ignore temperature data
    rename(
      datetime = 1, Nitrate_mg = 2, Nitrate_mg_status = 3, TurbidityRawFTU = 4,
      TurbidityRawFTU_status = 5
    ) %>%
    mutate(datetime = as.POSIXct(datetime, tz = "EST")) %>%
    arrange(datetime) %>%
    filter(
      is.na(TurbidityRawFTU_status),
...
clean_SCAN_data_since20230724 <- function(fpath, end_of_previous) {
  ...
  read_xlsx(fpath,
    ...
    rename(
      datetime = 1, Nitrate_mg = 2, Nitrate_mg_status = 3, TurbidityRawNTU = 4,
      TurbidityRawNTU_status = 5
    ) %>%
    mutate(datetime = as.POSIXct(datetime, tz = "EST")) %>%
    arrange(datetime) %>%
    filter(
      datetime > end_of_previous,
...
dbWriteTable(con,
  ...
  insert_df,
  append = TRUE
) # do not set to FALSE! read the first comment in this script

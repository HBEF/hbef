shinyServer(function(input, output, session) {
    ...
    sensor_data <- reactive({
        ...
        #var <- sensor_name_map[sensor_name_map == var]
    ...
    output$TITLE4 <- renderText({
        ...
        var <- names(sensor_name_map[sensor_name_map == var])

        site <- str_split_fixed(site, '', n = Inf)[1,2]
        title <- glue('{v} for Watershed {w}',
                      v = var,
                      w = site)
    ...
    output$GRAPGH4 <- renderDygraph({
      ...
      if(nrow(sensor_data) == 0){
          ...
          if(!is.null(grab_add) && grab_add == "Overlay grab samples"){
              ...
              #Assemble y lable 
              sensor_name <- names(sensor_name_map[sensor_name_map == var])
              sensor_unit <- unname(sensor_unit_map[sensor_name_map == var])
            ...
            date_mid <- base::as.Date((as.numeric(date[1]) + as.numeric(date[2])) / 2, 
                                        origin = '1970-01-01')
            ...
            dg <- dygraph(sensor_data_xts, group='group2') %>%
                ...
                dyOptions(drawGrid = FALSE,
                          ...
                          includeZero = TRUE,
                          colors = 'black') %>%
              dyAnnotation(x = date_mid, text = 'LIVE STREAMED PROVISIONAL DATA - NOT FOR PUBLICATION')
          }
      }
    })
})
